...
{
    internal class RecordManager : JsonManagerBase<Record>
    {
        // 同时还需要操作 Vehicle 文件 → 组合一个 VehicleManager 实例
        private readonly VehicleManager _vehicleManager = new();

        // RecordManager 自己管 Record 文件
        public RecordManager() : base("./RecordManager.json") { }

        // 租赁
        public (string, List<Record>) lease(int VehicleId, int UserId)
        {
            if (VehicleId <= 0 || UserId <= 0)
                return ("车辆ID或用户ID不能为空", new List<Record>());

            var (_, listRecord) = ReadData();          // ← 直接调用基类方法
            var (_, listVehicle) = _vehicleManager.ReadData();

            int findId = listVehicle.FindIndex(v => v.Id == VehicleId);
            if (findId < 0) return ("车辆不存在", new List<Record>());

            listVehicle[findId].Status = VehicleStatusEnum.Rented;
            listRecord.Add(new Record(VehicleId, UserId, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));

            WriteData(listRecord);                     // ← 基类写 Record
            _vehicleManager.WriteData(listVehicle);    // ← VehicleManager 写 Vehicle
        ...
        {
            if (VehicleId <= 0)
                return ("车辆ID不能为空", new List<Record>());

            var (_, listRecord) = ReadData();
            var (_, listVehicle) = _vehicleManager.ReadData();
            ...
            int idx = listRecord.FindIndex(r => r.VehicleId == VehicleId && r.ReturnTime == null);
            if (idx < 0) return ("未找到租赁记录", new List<Record>());

            listRecord[idx].ReturnTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            DateTime.TryParse(listRecord[idx].LeaseTime, out DateTime leaseTime);
            listRecord[idx].Pay = (DateTime.Now - leaseTime).TotalHours * listVehicle[findId].Price;

            WriteData(listRecord);
            _vehicleManager.WriteData(listVehicle);
