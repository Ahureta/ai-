...
{
    internal class VehicleManager : JsonManagerBase<Vehicle>
    {
        // 调用基类构造函数，传入文件路径
        public VehicleManager() : base("./CarManager.json") { }

        // ---- 车辆类型枚举展示（和文件读写无关，原样保留）----
        public void SearchAllVehicleTypes()
        {
            Console.WriteLine("车辆类型：");
            foreach (var type in System.Enum.GetValues(typeof(VehicleTypeEnum)))
            {
        ...
        {
            if (!System.Enum.TryParse<VehicleTypeEnum>(type, true, out var vehicleType))
        ...
        {
            (_, List<Vehicle> list) = ReadData();
            List<Vehicle> freeList = list.FindAll(v => v.Status == VehicleStatusEnum.Available);
            string msg = freeList.Count == 0 ? "列表为空,暂无车辆" : "读取成功";
            return (msg, freeList);
        }
        ...
        internal (string, List<Vehicle>) SearchOne(int id)
        {
            ...
            (_, List<Vehicle> list) = ReadData();
            if (list.Count == 0) return ("暂无车辆数据", new List<Vehicle>());
...
//        #region    //对于反射的使用太耗性能,一般不用.在基类上声明子类的类型就好了
...
//        {
//            Console.WriteLine("车辆类型：");
//            foreach (var type in Enum.GetValues(typeof(VehicleTypeEnum)))
//            {
//                Console.WriteLine($"- {type}");
//            }
//        }
